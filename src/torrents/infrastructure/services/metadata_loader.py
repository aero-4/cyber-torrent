...
class MetadataParser:
    ...
    async def search_games(self, page: int = 1, max_size: int = 100, platform: str = "1"):
        self.params.update(platforms=platform,
                           page_size=max_size,
                           page=page)
        return await self.request(self.params)

    async def get_details(self, slug: str):
        url = self.base_url + f"/{slug}"
        return await self.request(self.params, url)

    async def request(self, params: dict, url: str = None, timeout: float = 10.0):
        async with aiohttp.ClientSession(timeout=ClientTimeout(timeout)) as client:
            try:
                r = await client.get(url or self.base_url, params=params)
                r.raise_for_status()
                data = await r.json()
